...
class MemoryDreamer:
    ...
    def check_trigger(self, force: bool = False) -> DreamingDecision:
        ...
        raw_session_items = [item for item in items if _is_raw_session(item)]
        eligible_sessions = [
            item
        ...
        if state.get("last_dream_at") and eligible_sessions and hours_since_last >= max(1, self.config.interval_hours):
            return DreamingDecision(True, "time_interval_with_eligible_sessions", metrics)
    ...
    def run(self, force: bool = False) -> DreamingResult:
        ...
        state = _read_state(self.state_path)
        try:
            duplicate_archive_ids = _duplicate_archive_ids(
                _dedup_eligible_items(self.memory_store.all(), self.config.session_hot_days)
            )
            ...
            candidates, token_usage, skipped, archive_ids = self._reflect(batch)
            ...
            semantic_archive_ids = [
                ...
                if memory_id in written_parent_ids and _is_archivable_long_term(memory_id, batch)
            ]
            session_archive_ids = [
                memory_id
            ...
            if semantic_archive_ids:
                result.archived.extend(
                    self.memory_store.archive(semantic_archive_ids, reason="dreaming_semantic_merge")
                )
            ...
            processed_items = [
                item
                for item in batch
                if item.memory_type in LONG_TERM_MEMORY_TYPES or item.memory_id in archived_session_ids
            ]
    ...
    def _select_batch(self, state: dict[str, Any]) -> list[MemoryItem]:
        items = self.memory_store.all()
        processed_session_ids = set(_as_string_list(state.get("processed_session_ids")))
        ...
        eligible_sessions = [
            item
            for item in items
            if _is_raw_session(item)
            and item.memory_id not in processed_session_ids
            and _is_older_than_days(item, self.config.session_hot_days)
        ]
        long_terms = [item for item in items if item.memory_type in LONG_TERM_MEMORY_TYPES]

        batch: list[MemoryItem] = []
        batch.extend(eligible_sessions[:limit])
        remaining = limit - len(batch)
        if remaining > 0:
            batch.extend(_diverse_long_terms(long_terms, remaining))
        return batch
    ...
    def _reflect(
        ...
        messages = [
            {"role": "system", "content": _dreaming_prompt(max_candidates=max(1, self.config.max_batch_size // 2))},
            {
                "role": "user",
                "content": json.dumps(
                    {
                        ...
                        "instructions": [
                            "Merge duplicates and overlapping memories.",
                            "For old raw session memories, create one or more session_memory candidates with subtype=session_summary.",
                            "Separately decide whether any durable project/procedural/experience memories should be created.",
                            "Keep recent raw session memories out of the batch; only summarize the provided old raw sessions.",
                        ],
                    },
                    ensure_ascii=False,
                ),
        ...
        for raw in raw_candidates:
            ...
            if bool(raw.get("sensitive", False)):
                ...
                continue
            if memory_type == "session_memory" and subtype != SESSION_SUMMARY_SUBTYPE:
                skipped.append({"title": title, "type": memory_type, "reason": "invalid_session_subtype"})
                continue
            if memory_type not in LONG_TERM_MEMORY_TYPES and memory_type != "session_memory":
                skipped.append({"title": title, "type": memory_type, "reason": "invalid_memory_type"})
                continue
            if memory_type == "session_memory" and not any(_is_raw_session(batch_by_id[memory_id]) for memory_id in source_ids):
...
def _dreaming_prompt(max_candidates: int) -> str:
    return (
        "You are MiniCode's offline memory dreaming pass. Return one JSON object only. "
        "Consolidate the provided old session memories and long-term memories. "
        "Allowed outputs are: "
        "1) session_memory with subtype=session_summary, used to replace old raw session memories in the active session layer; "
        "2) project_memory, procedural_memory, or experience_memory, used only for durable long-term knowledge. "
        "Do not output raw session_memory without subtype=session_summary. "
        ...
        "Only include archive_memory_ids for long-term memories that are fully superseded by a new candidate; "
        "do not include session memories in archive_memory_ids. Old raw session memories are archived by MiniCode "
        "only after a session_summary candidate is successfully written. "
        "Schema: "
...
def _diverse_long_terms(items: list[MemoryItem], limit: int) -> list[MemoryItem]:
    ...
    while len(selected) < limit and by_type:
        progressed = False
        for memory_type in sorted(list(by_type)):
            if not by_type[memory_type]:
                del by_type[memory_type]
                continue
            selected.append(by_type[memory_type].pop(0))
            progressed = True
...
def _is_archivable_long_term(memory_id: str, batch: list[MemoryItem]) -> bool:
    for item in batch:
        if item.memory_id == memory_id:
            return item.memory_type in LONG_TERM_MEMORY_TYPES
    return False
